...
def fetch_binance_klines(symbol: str, interval: str, limit: int = 200) -> pd.DataFrame:
    try:
        ...
        logger.exception(f"Error fetching Binance data for {symbol}: {e}")
        raise
...
def fetch_bybit_klines(symbol: str, interval: str, limit: int = 200) -> pd.DataFrame:
    try:
        ...
        if "result" in jd and isinstance(jd["result"], list):
            data = jd["result"]
        elif "result" in jd and isinstance(jd["result"], dict) and "list" in jd["result"]:
            data = jd["result"]["list"]
        else:
            data = jd.get("result", [])
        if not data:
        ...
        logger.exception(f"Error fetching Bybit data for {symbol}: {e}")
        raise
...
def pivot_support_resistance(series: pd.Series) -> Dict[str, float]:
    try:
        ...
        logger.exception(f"Error calculating pivot: {e}")
...
def fib_levels(last_low: float, last_high: float) -> Dict[str, float]:
    try:
        ...
        logger.exception(f"Error calculating Fibonacci levels: {e}")
        return {}
...
def score_signals(ind_df: pd.DataFrame) -> Tuple[str, Dict[str,int]]:
    try:
        ...
        logger.exception(f"Error scoring signals: {e}")
...
def build_hourly_report(symbol: str) -> Tuple[str, bytes]:
    ...
    for tf_label, tf_conf in TIMEFRAMES.items():
        try:
            ...
            logger.exception(f"Error building report for {symbol} {tf_label}: {e}")
...
def telegram_send_text(text: str):
    try:
        ...
        logger.info(f"Telegram send text status: {r.status_code}")
        return r.status_code, r.text
    except Exception as e:
        logger.exception("Telegram send message error:")
        return None, str(e)
...
def telegram_send_photo(image_bytes: bytes, caption: str = ""):
    try:
        ...
        logger.info(f"Telegram send photo status: {r.status_code}")
        return r.status_code, r.text
    except Exception as e:
        logger.exception("Telegram send photo error:")
        return None, str(e)
...
def hourly_task():
    for sym in SYMBOLS:
        try:
            logger.info(f"Running hourly task for {sym}")
            text, img = build_hourly_report(sym)
            telegram_send_text(text[:4000])
            if img:
                caption = f"{sym} hourly chart (simulation)"
                telegram_send_photo(img, caption=caption)
            # short sleep to avoid Telegram rate limits
            time.sleep(1.0)
        except Exception as e:
            logger.exception("Error in hourly_task")

def daily_task():
    for sym in SYMBOLS:
        try:
            logger.info(f"Running daily task for {sym}")
            text, img = build_hourly_report(sym)
            ...
            logger.exception("Error in daily_task")
...
# extra explicit HEAD handler that returns empty 200 (some monitors expect empty body)
...
async def health_check():
    try:
        # Test Binance API connectivity with a small request
